...
public class ShopService {
    ...
    @Autowired
    public ShopService(Authenticator authenticator, ProductService
            productService) {
    ...
    public Long openOrder(Long userId) {
        if (authenticator.isAuthenticated(userId)) {
            Long maxOrder = orders.stream()
                    ...
                    .max()
                    .orElse(0);
            orders.add(new Order(maxOrder + 1, userId,
                    productService));
            return maxOrder + 1;
        } else {
            return -1L;
    ...
    public void addItem(Long orderId, Long productId, double qty) {
        orders.stream()
                ...
                .forEach(o -> o.getItems().add(new Item(productId,
                        qty)));
    }
    ...
    public boolean removeItem(Long orderId, Long productId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            {
                if
                (theOrder.getItems().get(n).getProductId().equals(productId));
                {
